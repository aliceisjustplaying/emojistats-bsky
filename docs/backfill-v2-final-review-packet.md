# Backfill v2 Final Review Packet

Generated: 2026-06-15 UTC

This packet collects the material needed to review the backfill v2 spec and the decisions resolved in the second grilling session.

## Review Order

1. Resolved decisions from the second grill.
2. Remaining critique checklist.
3. Current project glossary in CONTEXT.md.
4. ADR 0001.
5. Current design record.
6. Retro source material.

---

## Resolved Decisions From Second Grill

- Product boundary: keep a private Raw Archive as operational truth. The candidate Published Raw Observed Corpus contains raw post text plus full record extras, but final public filtering/release policy is decided later.
- Corpus semantics: the public raw corpus is an Observed Corpus, not cumulative-ever. It contains repository records present at crawl time; records deleted before observation are unrecoverable.
- Deletes and updates: post deletes are not observed. Live update events are protocol-possible, but current Bluesky behavior relied on here is creates only.
- Live/archive split: the Published Raw Observed Corpus is backfill snapshot only. Jetstream Catch-Up is serving-site only and writes directly to ClickHouse, not to the Raw Archive.
- Jetstream catch-up: record backfill start time, finish the backfill, then consume Jetstream from backfill_started_at minus four hours. Public Jetstream retention was spot-checked at about 36 hours on 2026-06-15, and the run target is 24 hours.
- Local Jetstream fallback: if the project slips beyond the 24-hour target, start a self-operated Jetstream server.
- Launch definition: the 100% Website requires backfill, derive, Jetstream Catch-Up to live tail, and aggregate rebuild before public launch.
- ClickHouse identity: Parquet and receipts include CID; ClickHouse serving rows omit CID and use the smaller serving identity because ClickHouse is not the archive truth.
- Aggregate rebuild: keep a final aggregate rebuild after Jetstream Catch-Up to remove duplicate-count effects from materialized views. It must be canary-measured; minutes to low hours is acceptable, days is not.
- Archive fidelity: archive rows are Data-Model Lossless, not byte-for-byte CAR/CBOR lossless. Keep record CID, typed fields, and canonical extras JSON.
- Timestamp semantics: store raw/normalized created_at plus parse status; package partitions must handle valid, missing, invalid, and future timestamps explicitly.
- Verification claim: v2 claims Snapshot Completeness only, not authorship or identity verification. Signature/identity verification fields should record false/skipped unless implemented later.
- listRecords fallback: Collection-Paginated Records support serving projection only and do not carry Snapshot Completeness. They must not be mixed with root-proofed snapshot counts.
- Receipts: per-repo receipts hash full canonical row content, not just rkeys. Collection is an implicit constant for app.bsky.feed.post.
- Resource limits: use Loud Resource Caps, not no caps. Limits must produce explicit status, metrics, and recovery/operator-action paths.
- Storage Box manifest: use a committed-manifest protocol. Canary may choose checksum sidecars instead of full remote readback if overhead is high.
- Canary: use a Stratified Canary with random, whale, old/future/invalid timestamp, emoji-heavy, third-party/Bridgy, malformed/drop/partial-upload cases, plus a short multi-box contention test.
- did:web: coverage is best-effort from known PDS listings, live observations, and manual seeds; no global census claim.
- listRepos absence: do not classify absent-from-host from a partial or suspect listing. Trust it only after the host listing finishes cleanly and identity says that host is authoritative.
- Host policy: no separate HostPolicy file for v2. Use built-in header pacing, 429/backoff, and global kill switches.
- Normalizer: write Normalizer Version metadata wherever rows are produced.
- Profile sidecar: include app.bsky.actor.profile/self from the same repo fetch as archive metadata; no handle verification, media fetches, or ClickHouse profile search.
- ADR: record the Raw Archive / Published Raw Observed Corpus boundary in docs/adr/0001.

---

## Critique Checklist To Apply To The Design Record

- Replace locked status with a status that says architecture direction accepted, with publication policy and selected safety details still pending until incorporated.
- Replace raw HuggingFace publication certainty with private Raw Archive plus candidate Published Raw Observed Corpus and deferred publication review.
- Replace cumulative-ever language with Observed Corpus language.
- Replace bounded only by mushroom PDS rate limits with primarily bounded by those limits after canary proves parse/archive/derive/ClickHouse/rebuild are not long poles.
- Replace no size cap with no silent content cap / Loud Resource Caps.
- Narrow MST proof wording to Snapshot Completeness, separate from signature and identity verification.
- Upgrade receipts from rkey digest to canonical row-content hashes.
- Remove same-table live/backfill overlap from the happy path; use post-backfill Jetstream Catch-Up instead.
- Label listRecords fallback as collection-paginated only and exclude it from Snapshot Completeness counts.
- Specify Storage Box committed-manifest semantics.
- Make canary stratified and require measured aggregate rebuild time.
- Compute authoritative status from committed snapshots/manifests/receipts, not ad-hoc reads of live crawler state.
- Make crawler watchdog use a progress vector, not archived count alone.
- Treat ClickHouse insert dedupe token as an optimization; correctness comes from content hashes, batch ledgers, and rebuilds.
- Add did:web best-effort caveat.
- Add guardrails before terminal absent-from-host classification from listRepos.
- Define canonical extras JSON and Data-Model Lossless semantics.
- Add invalid/future created_at handling.
- Add Normalizer Version metadata to manifests, receipts, derive metadata, and live writer logs.
- Clarify profile sidecar fields and handle/media limits.
- Fix pix flake wording if it is not meant as project-specific infrastructure.
...
## Source: docs/adr/0001-raw-archive-and-public-corpus-boundary.md
...
## Source: docs/backfill-v2-design.md
