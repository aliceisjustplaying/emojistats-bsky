...
This packet collects the material needed to review the backfill v2 spec and the decisions resolved in the second grilling session. The embedded design record has been mechanically reconciled with these decisions.

## Review Order

1. Resolved decisions from the second grill and follow-up critique.
2. Acceptance checklist now reflected in the design record.
3. Current project glossary in CONTEXT.md.
4. ADR 0001.
5. Current reconciled design record.
6. Retro source material.

---

## Resolved Decisions Reflected In The Design Record
...
- Jetstream catch-up: record backfill start time, finish the backfill, then consume Jetstream from backfill_started_at minus four hours. Public retention is an operational dependency; a local Jetstream/spooler must start before retention can no longer cover that rewind point, or launch wording downgrades from 100%.
...
- Host overrides: v2 needs a per-host operator escape hatch, but it can be a CLI/table/Nix surface rather than a standalone HostPolicy file.
...
- Rust rewrite: proceed vertical-slice-first with fetch-one before fleet scheduler work.

---

## Acceptance Checklist Reflected In The Design Record

- Locked status replaced with architecture-direction-accepted status.
- Raw HuggingFace certainty replaced with private Raw Archive plus candidate Published Raw Observed Corpus and deferred publication review.
- Cumulative-ever language replaced with Observed Corpus language.
- Mushroom-rate-limit win condition constrained by canary proof that parser/archive/derive/ClickHouse/rebuild are not long poles.
- No-size-cap language replaced with Loud Resource Caps.
- MST proof wording narrowed to Snapshot Completeness.
- Rkey digest replaced with canonical row-content hashes.
- Same-table live/backfill overlap removed from the happy path; post-backfill Jetstream Catch-Up is used instead.
- listRecords fallback labeled as collection-paginated only and excluded from Snapshot Completeness counts.
- Storage Box committed-manifest semantics specified.
- Canary made stratified and aggregate rebuild time added as a gate.
- Authoritative status moved to committed snapshots/manifests/receipts.
- Watchdog changed to a progress vector.
- ClickHouse insert dedupe token treated as an optimization.
- did:web best-effort caveat added.
- listRepos absence guardrails added.
- Canonical extras JSON and Data-Model Lossless semantics added.
- Invalid/future created_at handling added.
- Normalizer Version metadata added.
- Profile sidecar fields and handle/media limits clarified.
- pix flake terminology defined.
- fetch-one made the first Rust implementation milestone.
