...
### 07:37 UTC — parse-pool deadlock FIX deployed fleet-wide (1008a03)
Root-cause fix for the recurring scheduler wedge is LIVE on all 6 boxes (rolling restart, all rev 1008a03). Fix: per-job reply-timeout backstop in parse-pool (REPO_FETCH_TIMEOUT_MS+300s=600s) frees leaked GLOBAL_CONCURRENCY slots when a worker never replies. Codex-reviewed gpt-5.5 xhigh, 3 rounds, logic approved; 15 tests pass, 0 lint. VALIDATION WINDOW STARTS NOW (~07:37): crawl1 was wedging every ~10-15min; if it stays quiet 20-30min, fix confirmed. Watchdog v3 remains as backstop. emoji clean (392 parts, 0 delayed). Remaining 12,372,592; ETA ~5.6h → ~13:15 UTC (this window's rate depressed by the fleet-wide restart). shard1 678k, shard2 779k — both nearing 500k retire-flag, not yet crossed.
### 07:43 UTC — fix partially effective: crawl1 drain-phase leak outpaces 600s timeout
crawl1 wedged ~7min after the 1008a03 restart (age 205s, 0 CPU, inFlight=2787 stuck, backlog=0, zero reply-timeout firings — leaked slots <600s old, timers not expired). The 600s reply-timeout is too slow vs crawl1's drain-phase leak rate: shard1 nearly drained (678k, backlog 0), remaining repos skew to slow/stalling hosts → high leak rate → wedge forms in ~7min, before any 600s timer fires. Watchdog covers it. OPEN QUESTION: do the other (lower-churn) boxes stay wedge-free with 600s? If yes, only crawl1's drain phase is affected (and it retires soon); if they also wedge, lower REPLY_TIMEOUT_MS toward ~360s (just above fetcher's 300s fetch cap) — gather 1-2 cycles of multi-box wedge data before changing, avoid thrash. For Alice: the durable fix may be both a shorter reply-timeout AND fixing why fetches stall past the 300s AbortSignal (the residual leak source).
### 07:51 UTC — DECISION: hold at 600s; crawl1 stuck in drain-phase thrash
HONEST STATUS: crawl1 (shard1, 679k) has emitted NO telemetry since 07:37 — it thrash-wedges within ~2min of each watchdog restart. Cause: shard1's drain tail is almost all STALLING hosts; every claim leads to a stalled getRepo (fetch-abort fails to fire), leaking a slot, wedging near-instantly. The 600s reply-timeout fix cannot help (slots leak faster than 600s frees them; reply_timeouts=0 fleet-wide confirms it has not fired). crawl2 (shard2, 724k) similar but milder.
DECISION: HOLD at 600s, do NOT lower the timeout. Reasoning: a shorter timeout would not save crawl1's startup wedge (leak is too fast) AND could worsen drain-phase churn (slow-host repos retry transiently forever in waves). Net effect uncertain → conservative rule = no change. One fix already deployed; a 2nd speculative one unsupervised is over-reaching.
FOR ALICE (root-cause fix, needs supervision): the durable fix is fetcher-level — make stalled getRepo sockets abort fast (the 300s AbortSignal sometimes never fires on dead sockets) so they register as timeout failures and dead-host detection parks the stalling hosts; then crawl1 stops claiming them and stops leaking. Possibly pair with a shorter REPO_FETCH_TIMEOUT_MS. crawl1/crawl2 are the 2 smallest shards (near retire); their stuck ~1.4M is largely unreachable repos on dead hosts that the final verify/sweep would classify terminal anyway.
BULK PROGRESSING: crawl0/3/4/5 (~10.6M) healthy at ~23k/min ≈ 7.6h. Watchdog covers crawl1/crawl2 thrash. emoji clean (375 parts, 0 delayed).
### 08:04 UTC — ETA cycle; crawl1 draining slowly (not fully stuck)
All six active+fresh. crawl1 emitted telemetry this window: shard1 679k→660k, so it DOES drain between wedges (14k/sec in productive windows), just interrupted. shard2 696k. Both nearing 500k retire-flag. Bulk crawl0/3/4/5 healthy (4096 fetching). emoji clean (384 parts, 0 delayed). Per-shard resolved/min (07:52-08:04, crawl1/crawl2-depressed): s0 3739, s1 3867, s2 3060, s3 4391, s4 3730, s5 4288 = 23,075/min windowed. Remaining 11,724,032. ETA: windowed ~8.5h but thrash-depressed; ~5.6h at healthy 35k. Realistic ~7-8h (~15:00-16:00 UTC) given ongoing crawl1/crawl2 drag. Holding at 600s, no code changes.
### 08:16 UTC — fleet rate declining (end-game tail)
Fleet rate trend DOWN: 40k(06:26)->37k->29k->23k->20k/min now. NOT just crawl1/crawl2 thrash — bulk crawl0/3/4/5 also slowed to 3.3-3.9k/min each. This is normal backfill end-game: easy repos done, remaining ~11.5M skew to slower hosts + more retries + bigger CARs. emoji clean (388 parts, 0 delayed), all boxes healthy/active — slowdown is remote-host-bound, NOT our infra, nothing to fix. crawl3 spiked 12.0G (whale, fresh+resolving). Per-shard resolved/min (08:04-08:16): s0 3645, s1 3100, s2 2701, s3 3304, s4 3699, s5 3912 = 20,361/min. Remaining 11,524,951. ETA at current rate ~9.4h (~17:30 UTC); likely stretches further as tail hardens, but could improve if tail isn't uniformly slow. shard1 656k, shard2 675k still >500k. Holding, no code changes.
### 08:28 UTC — ETA cycle; rate stabilized ~20k
Fleet rate stabilized ~20k/min (08:16 20.4k -> 08:28 19.8k, no longer free-falling). All six active+fresh; crawl1 hit 18k/sec in a productive window (draining shard1 well between wedges). Bulk crawl0/3/4/5 at 3.3-4.0k/min each (slow-tail rate). emoji clean (401 parts, 0 delayed). Per-shard resolved/min (08:16-08:28): s0 3775, s1 2124, s2 2827, s3 3811, s4 3303, s5 3966 = 19,806/min. Remaining 11,298,104. ETA ~9.5h → ~18:00 UTC. shard1 644k, shard2 642k both >500k, slowly approaching. Holding, no code changes.
### 08:40 UTC — ETA cycle; shard1 nearing 500k
All six active. crawl1 productive (11.8k/sec, shard1 draining well); crawl2 wedged awaiting cooldown (expected). Bulk crawl0/3/4/5 healthy. emoji clean (404 parts, 0 delayed). Per-shard resolved/min (08:28-08:40): s0 3705, s1 4626, s2 1538, s3 3967, s4 3607, s5 3282 = 20,725/min. Remaining 11,061,850. ETA ~8.9h → ~17:30 UTC. shard1 594k (closest, ~20min to 500k ~09:00), shard2 628k. Holding.
### 08:51 UTC — ETA cycle; shard1 539k (retire-flag imminent)
All six active, bulk healthy, crawl1/crawl2 thrash watchdog-managed. emoji clean (389 parts, 0 delayed). Per-shard resolved/min (08:40-08:51): s0 3664, s1 4952, s2 1901, s3 4157, s4 3199, s5 3866 = 21,739/min. Remaining 10,823,550. ETA ~8.3h → ~17:10 UTC. shard1 539k — will cross 500k retire-flag ~09:00 next cycle; shard2 609k next. Holding.
### 09:00 UTC — RETIRE-FLAG: shard1 < 500k
shard1 (crawl1) pending+fetching = 496,790, crossed the 500k retire-flag. READY FOR ALICE TO RETIRE crawl1 manually. Caveat: crawl1 is the drain-phase thrash box, so its ~497k drains slowly (much is unreachable/dead-host repos); retiring it stops paying for a box mostly fighting its stalling tail — the residual real repos get classified by the final verify/sweep. shard2 (crawl2) 601k next (~20min). Other shards: s0 1.83M, s5 2.43M, s3 2.59M, s4 2.70M.
### 09:01 UTC — CORRECTION: shard1 remaining is REAL, do NOT retire crawl1
Alice rightly pushed back on retiring at 500k. Checked shard1 ledger: of 491k pending, the top hosts are ALL healthy Bluesky mushrooms (lionsmane/enoki/inkcap/shiitake/oyster/shimeji/porcini/amanita .us-east.host.bsky.network, ~24-30k repos each). These are REAL fetchable repos, NOT dead-host residue — my earlier 'much is unreachable' claim was WRONG (inferred, not measured). crawl1's drain-phase thrash is therefore NOT dead hosts; it's CONTENTION on the big mushroom hosts (shard1's tail is concentrated on them; concurrency 4096 hammering them → rate-limit/slow responses → stalled fetches → slot leak → wedge). Conclusion: do NOT retire crawl1 — it has ~490k real repos to finish; it'll grind through them via watchdog-restart cycles. The '500k retire-flag' concept was misguided; retire only when pending+fetching ~0. NOTE for Alice: crawl1's contention-wedge might ease at LOWER concurrency on that box (fewer simultaneous hits on rate-limited mushrooms) — a possible lever, but concurrency is locked at 4096 per your call; flagging as an option, not acting.
### 09:14 UTC — ETA cycle
...
### 09:47 UTC — ETA cycle
All six active+fresh. emoji clean (412 parts, 0 delayed). Per-shard resolved/min (09:36-09:47): s0 3757, s1 1073, s2 5113, s3 8952, s4 3840, s5 3770 = 26,505/min. Remaining 9,556,051. ETA ~6.0h → ~15:50 UTC. shard1 373k, shard2 421k. Holding.
### 09:58 UTC — ETA cycle
All six active+fresh; crawl3 12.2G (whale, resolving). emoji clean (413 parts, 0 delayed). Per-shard resolved/min (09:47-09:58): s0 3887, s1 4937, s2 4399, s3 4130, s4 3116, s5 3483 = 23,952/min. Remaining 9,296,585. ETA ~6.5h → ~16:30 UTC. shard1 319k, shard2 372k. Holding.
### 10:02 UTC — crawl2 log-flood + legit wedge restart
crawl2 alert showed age=99999 (watchdog sentinel for 'no crawl-stats line in last 400 journal lines'). Cause: a flood of 'repo failed: max attempts on a non-transient error' (~1195 in 2min) from shard2's drain tail pushed stats lines out of the 400-line window. The CPU check (0 ticks/s) still correctly confirmed a real wedge → legit auto-restart. Watchdog robust (CPU gate prevents false-restart of busy boxes; no fix needed). Those failures = shard2 draining to 'failed' terminal on genuinely-bad/absent repos (non-transient), expected tail behavior, not data loss. crawl2 recovered.
### 10:09 UTC — ETA cycle
