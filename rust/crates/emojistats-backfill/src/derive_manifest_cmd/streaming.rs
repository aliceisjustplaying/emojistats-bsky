use emojistats_backfill::{
    ...
    derive::{
        BACKFILL_DERIVE_SOURCE, DeriveCheckpointKey, PostServingRow, TotalPostCounterInput,
...
impl<'a> CanonicalStreamingPayloadState<'a> {
    ...
    fn flush_post_chunk(&mut self) -> anyhow::Result<ClickHouseInsertPayload> {
        ...
        let chunk_index = self.post_chunk_index;
        let token =
            canonical_streaming_post_dedupe_token(&self.verified.identity, chunk_index, &rows)?;
        increment(
